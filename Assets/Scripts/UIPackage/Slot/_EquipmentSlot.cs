...
/// <summary>
/// 角色面板的物品槽类，继承自物品槽基类.（有特殊限制的Slot：只能存放装备），不同格子只能存放这个格子应该有的装备。
/// </summary>
public class _EquipmentSlot : _Slot {

    public _Equipment.EquipmentType equipmentSoltType;//角色装备里每个物品槽的物品类型。因为每个物品槽能装备的物品类型不同，所以才分类型，比如说，头部只能放头部，腿部只能放腿部之类的。主手只能放武器。
    public _Weapon.WeaponType wpType;//武器类型，只有副手和主手能有武器，其他为Null
    ...
    public override void OnPointerDown(UnityEngine.EventSystems.PointerEventData eventData)//重写父类Slot的方法
    {
        ...
        {
            if (transform.childCount > 0 && _InventroyManager.Instance.IsPickedItem == false)//需要脱掉的物品得有，并且鼠标上要没有物品，否则就发生：当鼠标上有物品，在其他物品上点击鼠标右键也能脱掉这种情况。
            {
                _ItemUI currentItemUI = transform.GetChild(0).GetComponent<_ItemUI>();
                _Item item = currentItemUI.Item;//临时保存物品信息，防止下面一销毁就没了
        ...
        if (_InventroyManager.Instance.IsPickedItem == true)//一：鼠标上有物品
        {
            _ItemUI pickedItemUI = _InventroyManager.Instance.PickedItem;//鼠标上的物品
            if (transform.childCount > 0)//1.当前装备槽有装备
            {
                _ItemUI currentItemUI = transform.GetChild(0).GetComponent<_ItemUI>();//当前物品槽中的物品
            ...
            else//2.当前装备槽无装备
            {
                if (IsRightItem(pickedItemUI.Item))//如果鼠标上的物品是否适合放在该位置，那就把该物品放入角色面板，并且鼠标上的物品减少一个，否则不做处理
                {
                    ...
                    _InventroyManager.Instance.ReduceAmountItem(1);
                    isUpdataProperty = true;//需要更新角色属性
                }
            }
        }
        else//二：鼠标上没有物品
        {
            ...
            {
                _ItemUI currentItemUI = transform.GetChild(0).GetComponent<_ItemUI>();
                _InventroyManager.Instance.PickUpItem(currentItemUI.Item, currentItemUI.Amount);
