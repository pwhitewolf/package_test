...
/// 物品槽基类
/// </summary>
public class _Slot : MonoBehaviour,IPointerEnterHandler,IPointerExitHandler,IPointerDownHandler {
    ...
    //根据索引器得到当前物品槽中的物品类型
    public _Item.ItemType GetItemType()
    {
        return transform.GetChild(0).GetComponent<_ItemUI>().Item.Type;
    }

    //根据索引器得到当前物品槽中的物品ID
    public int GetItemID()
    {
        return transform.GetChild(0).GetComponent<_ItemUI>().Item.ID;
    }

    //判断物品个数是否超过物品槽的容量Capacity
    public bool isFiled()
    {
        _ItemUI itemUI = transform.GetChild(0).GetComponent<_ItemUI>();
    ...
    public void OnPointerEnter(PointerEventData eventData)
    {
        ...
        {
            string toolTipText = this.transform.GetChild(0).GetComponent<_ItemUI>().Item.GetToolTipText();
            _InventroyManager.Instance.ShowToolTip(toolTipText);//显示提示框
        }
    }
    //接口实现的方法，鼠标离开时触发
    public void OnPointerExit(PointerEventData eventData)
    {
        if (this.transform.childCount > 0)
        {
            _InventroyManager.Instance.HideToolTip();//隐藏提示框
        }
    }
    ...
    public virtual void OnPointerDown(PointerEventData eventData)//为虚函数，方便子类EquipmentSlot重写
    {
        ...
        {
            if (transform.childCount > 0 && _InventroyManager.Instance.IsPickedItem == false)//需要穿戴的物品得有，并且鼠标上要没有物品，否则就发生：当鼠标上有物品，在其他物品上点击鼠标右键也能穿上这种情况。
            {
                _ItemUI currentItemUI = transform.GetChild(0).GetComponent<_ItemUI>();
                if (currentItemUI.Item is _Equipment || currentItemUI.Item is _Weapon)//只有装备和物品才可以穿戴
                {
                    _Item currentItem = currentItemUI.Item;//临时存储物品信息，防止物品UI销毁导致物品空指针
                    ...
                    if (currentItemUI.Amount <= 0)//物品槽中的物品没有了
                    {
                        ...
                        _InventroyManager.Instance.HideToolTip();//隐藏该物品的提示框
                    }
                    _CharacterPanel.Instance.PutOn(currentItem);//直接穿戴
                }
            }
        }
        ...
        {
            _ItemUI currentItem = transform.GetChild(0).GetComponent<_ItemUI>();//取得当前的物品
            if (_InventroyManager.Instance.IsPickedItem == false)///2.pickedItem == null。如果当前没有选中任何物品，即当前鼠标上没有物品
            {
                if (Input.GetKey(KeyCode.LeftControl))////①按下Ctrl键，取得当前物品槽中物品的一半
                {
                    ...
                    _InventroyManager.Instance.PickUpItem(currentItem.Item, amountPicked);
                ...
                else      ////②没有按下Ctrl键，把当前物品槽里面的物品放到鼠标上
                {
                    ...
                    _InventroyManager.Instance.PickUpItem(currentItem.Item, currentItem.Amount);
            ...
            else
            {
                ...
                ///////b.只能放下其中一部分
                ////②自身的id != pickedItem.id，pickedItem跟当前物品交换
                if (currentItem.Item.ID == _InventroyManager.Instance.PickedItem.Item.ID)
                {
                    if (Input.GetKey(KeyCode.LeftControl))
                    {
                        if (currentItem.Item.Capacity > currentItem.Amount)//如果物品槽里的物品容量大于当前物品数量，那就说明物品槽里还能再放
                        {
                            ...
                            _InventroyManager.Instance.ReduceAmountItem();//让鼠标上的物品减掉一个(默认移除一个)
                        }
                    ...
                    else
                    {
                        if (currentItem.Item.Capacity > currentItem.Amount)//如果物品槽里的物品容量大于当前物品数量，那就说明物品槽里还能再放
                        {
                            ...
                            if (itemRemain >= _InventroyManager.Instance.PickedItem.Amount)//如果物品槽还可以再放的数量大于等于鼠标上的数量，                                那就可以完全放下
                            {
                                currentItem.AddItemAmount(_InventroyManager.Instance.PickedItem.Amount);
                                _InventroyManager.Instance.ReduceAmountItem(itemRemain);
                            }
                            else//只能放下其中一部分
                            {
                                currentItem.AddItemAmount(itemRemain);
                                _InventroyManager.Instance.PickedItem.RemoveItemAmount(itemRemain);
                            }
                        }
                        else//如果物品槽里的物品容量小于当前物品数量，那就说明物品槽里不能再放了
                        {
                ...
                else//②自身的id != pickedItem.id，pickedItem跟当前物品交换
                {
                    ...
                    _Item pickedItemTemp = _InventroyManager.Instance.PickedItem.Item;
                    int pickedItemAmountTemp = _InventroyManager.Instance.PickedItem.Amount;

                    //保存当前物品槽中的物品，用于和鼠标上的物品交换
                    _Item currentItemTemp = currentItem.Item;
                    int currentItemAmountTemp = currentItem.Amount;
                    //两者交换
                    currentItem.SetItem(pickedItemTemp, pickedItemAmountTemp);//把当前鼠标上的物品放入物品槽
                    _InventroyManager.Instance.PickedItem.SetItem(currentItemTemp, currentItemAmountTemp);//把当前物品槽中的物品放在鼠标上
                }
            }
        }
        else//一：自身是空
        {
            ...
            ////②没有按下Ctrl键，放置当前鼠标上物品的所有
            ///2.pickedItem==null(即IsPickedItem == false)，不做任何处理
            if (_InventroyManager.Instance.IsPickedItem == true)
            {
                if (Input.GetKey(KeyCode.LeftControl))
                {
                    this.StoreItem(_InventroyManager.Instance.PickedItem.Item);
                    _InventroyManager.Instance.ReduceAmountItem();
                }
                else//②没有按下Ctrl键，放置当前鼠标上物品的所有
                {
                    for (int i = 0; i < _InventroyManager.Instance.PickedItem.Amount; i++)
                    {
                        this.StoreItem(_InventroyManager.Instance.PickedItem.Item);
                    }
                    _InventroyManager.Instance.ReduceAmountItem(_InventroyManager.Instance.PickedItem.Amount);
                }
            }
            else
            {
