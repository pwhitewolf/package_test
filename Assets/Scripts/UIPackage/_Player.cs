...
public class _Player : MonoBehaviour {
    ...
    void Start()
    {
        coinText = GameObject.FindGameObjectWithTag("Coin").GetComponentInChildren<Text>();
    ...
    void Update()
    {
        ...
        if (Input.GetKeyDown(KeyCode.G))
        {
            ...
            _Knapscak.Instance.StoreItem(id);
        }
        //按下B键控制背包的显示和隐藏
        if (Input.GetKeyDown(KeyCode.B))
        {
            _Knapscak.Instance.DisplaySwitch();
        }
        //按下H键控制箱子的显示和隐藏
        if (Input.GetKeyDown(KeyCode.H))
        {
            _Chest.Instance.DisplaySwitch();
        }
        //按下V键控制角色面板的显示和隐藏
        if (Input.GetKeyDown(KeyCode.V))
        {
            _CharacterPanel.Instance.DisplaySwitch();
        }
        //按下N键商店小贩面板的显示和隐藏
        if (Input.GetKeyDown(KeyCode.N))
        {
            _Vendor.Instance.DisplaySwitch();
        }
        //按下M键锻造面板的显示和隐藏
        if (Input.GetKeyDown(KeyCode.M))
        {
            _Forge.Instance.DisplaySwitch();
        }
    }
