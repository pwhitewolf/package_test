...
public class _Inventory : MonoBehaviour {
    ...
    protected _Slot[] slotArray;//存放物品槽的数组
    ...
    {//声明为虚函数，方便子类重写
        slotArray = GetComponentsInChildren<_Slot>();
        if (canvasGroupMy == null)
        {
            canvasGroupMy = GetComponent<CanvasGroup>();
        }
    ...
    {
        _Item item = _InventroyManager.Instance.GetItemById(id);
        return StoreItem(item);
    }

    //根据Item存储物品(重点)
    public bool StoreItem(_Item item)
    {
        if (item.ID == 0)
        {
        ...
        {
            _Slot slot = FindSameIDSlot(item);
    ...
    public void LoadInventory()
    {
        ...
        for (int i = 0; i < itemArray.Length - 1; i++)//长度减1是因为最后一个字符是 “-”，不需要取它
        {
            ...
            if (itemStr != "0")
            {
                ...
                _Item item = _InventroyManager.Instance.GetItemById(id);//通过物品ID得到该物品
